    ...
    confirmName = (username) => {
        ...
        if (username) {
            ...
            if (!userObj) { // 신규 계정인 경우
                ...
                window.sessionStorage.setItem("JITSIuser" + username, JSON.stringify(userObj));
            }
                    ...
                    { mentoData.map((mento, index) => (
                        <>
                            <tr height="40px">
                                ...
                                <td>
                                    { !friend.includes(mento.mento) && mento.mento != this.state.username &&
                                        <button className="follow-button" onClick={() => this._addFriendFromMento(mento.mento)}>+ 팔로우</button>
                                    }
                                    { friend.includes(mento.mento) &&
                                        "팔로우 중"
                                    }
    ...
    _sendMessage(messageObj, _valueName, _sender, _receiver, _text, divScroll) {
        ...
        if (this.state.Login) {
            if (this.state.messageWith) {
                ...
                else if (!_receiverObj.friend.includes(_sender)) {
                    console.log("receiver");
                    _receiverObj.friend = [
                        ..._receiverObj.friend,
                        _sender
