...
class WelcomePage extends AbstractWelcomePage {
    ...
    loadData = () => {
        //                      -------------- mentodata 처리 -------------
        let _mentoData = JSON.parse(window.localStorage.getItem("mentoData"));
        if (!_mentoData) { // DB 데이터가 없는 경우 기본 정보 + 계정정보를 추가
            ...
            window.localStorage.setItem("mentoData", JSON.stringify(_mentoData));
        }
        this.setState({
        ...
        //                      -------------- roomData 처리 -------------
        let roomObj = JSON.parse(window.localStorage.getItem("roomData"));
        if (!roomObj) {
            roomObj = [
            ...
            window.localStorage.setItem("roomData", JSON.stringify(roomObj));
        }
    ...
    confirmName = (username) => {
        ...
        if (username) {
            ...
            let userObj = JSON.parse(window.localStorage.getItem("JITSIuser" + username));
            if (!userObj) { // 신규 계정인 경우
                ...
                window.localStorage.setItem("JITSIuser" + username, JSON.stringify(userObj));
            }
    ...
    _mentoRegister = () => {
        ...
        window.localStorage.setItem("mentoData", JSON.stringify(mentoObj));
    }
                            ...
                            <tr height="40px">
                                ...
                                <td>
                                    ...
                                    { follow.includes(mento.mento) &&
                                        <button className="unfollow-button" onClick={() => this._removeFriendFromMento(mento.mento)}>언팔로우</button>
                                    }
    ...
    _addFriendFromMento = (newfriend) => {
        if (this.state.Login) {
            ...
            window.localStorage.setItem("JITSIuser" + name, JSON.stringify(userObj));
        }
        else {
            this.pageFlip(4);
        }
    ...
    _removeFriendFromMento = (newfriend) => {
        if (this.state.Login) {
            const { follow, name } = this.state.userData;
            const userObj = {
                ...this.state.userData,
                follow: [...follow.filter(_follow => _follow != newfriend)]
            }

            this.setState({ userData: userObj });
            window.localStorage.setItem("JITSIuser" + name, JSON.stringify(userObj));

            let _receiverObj = JSON.parse(window.localStorage.getItem("JITSIuser" + _receiver));

            if (_receiverObj && _receiverObj.follower.includes(_sender)) {
                _receiverObj.follower = [
                    ..._receiverObj.follower.filter(_follower => _follower != _sender)
                ]
            }
            window.localStorage.setItem("JITSIuser" + _receiver, JSON.stringify(_receiverObj));
        }
        else {
            this.pageFlip(4);
        }
