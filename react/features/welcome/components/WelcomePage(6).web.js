...
class WelcomePage extends AbstractWelcomePage {
    ...
    constructor(props) {
        ...
        this.state = {
            ...this.state,
            apiEndpoint: "https://meet3.2021sw3.tk/dbData/",
            generateRoomnames:
    ...
    componentDidMount() {
        ...
        if (userNameObj) {
            ...
            this.confirmName(userNameObj.name)
        }
    }
    ...
    loadData = () => {
        ...
        if (!_mentoData) { // DB 데이터가 없는 경우 기본 정보 + 계정정보를 추가
            _mentoData = [
                {mento: "정수환", info: "데이터베이스 가르쳐드립니다"},
                {mento: "박진석", info: "백준 풀이 도와드립니다. 저녁 시간에 연락주세요"},
                {mento: "홍승현", info: "프로그래밍 기초 과제풀이 도와드려요"},
                {mento: "김장군", info: "수학 정수론 등"},
                {mento: "이세연", info: "주식 관련 질문주세요"}
            ]
    ...
    confirmName = (username) => {
        ...
        if (username) {
            ...
            // let userObj = JSON.parse(window.sessionStorage.getItem("JITSIuser" + username));
            let userAccount = {};
            let userObj = null;
            console.log ("apiEndpoint = ", apiEndpoint + "userData.json");
            axios.get(apiEndpoint + "userData.json").then((Response)=>{
                console.log("Response : ", Response.data);
                userAccount = Response.data;
                if (username in userAccount) {
                    userObj = userAccount[username];
                    ...
                    console.log ("New userAccount : ", newUserAccount);
                    
                    const data = JSON.stringify(newUserAccount);
                    const options = {
                        headers: {"content-type": "application/json"}
                    }
                    axios.put(apiEndpoint + "userData.json", data, options).then((Response)=>{
                        console.log(Response);
                    }).catch(error=>{
                        console.log(error);
                    });;

                    // axios.put(apiEndpoint + "userData.json", newUserAccount).then((Response)=>{
                    //     console.log(Response);
                    // }).catch(error=>{
                    //     console.log(error);
                    // });
    ...
    _Message_With = (friendName) => {
        this.setState({ messageWith: friendName });
    }
                ...
                <div
                    ...
                    <div className="header-menu">
                        <div className="header-menu-items">
                            ...
                            <ul className="header-menu-ul">
                                <li className={`header-menu-li ${ this.state.Page == 1 && "bottom-red" }`} onClick={ () => this.pageFlip(1) }><p className="header-menu-li-p">멘토</p></li>
                                <li className={`header-menu-li ${ this.state.Page == 2 && "bottom-red" }`} onClick={ () => this.pageFlip(2) }><p className="header-menu-li-p">멘티</p></li>
                                <li className={`header-menu-li ${ this.state.Page == 3 && "bottom-red" }`} onClick={ () => this.pageFlip(3) }><p className="header-menu-li-p">팔로우</p></li>
                            </ul>
                            { !this.state.Login &&
                    ...
                    <>
                        ...
                        <div className="mento-section">
                            ...
                            { this._renderMento() }
                            { this.state.Login && this.state.mentoData.filter(mento => mento.mento == this.state.username).length == 0 &&
                    ...
                    <>
                    <div className = 'header'>
                        <h3 className='header-text'>더 많은 기능을 이용하기 위해 로그인 해주세요.</h3>
                    </div>
                        <div className="login-section">
                            ...
                            <input 
                                ...
                                value="로그인" 
                                onClick = { () => { this.loadData(); this.confirmName(this.state.username); this.pageFlip(1);} }>
                            </input>
                        </div>
                    </>
                    }
    ...
    _mentoRegister = () => {
        const { mentoInfoText, username, mentoData } = this.state;
        const mentoObj = [
            ...mentoData,
            { mento: username, info: mentoInfoText }
        ]
        this.setState({
            mentoData: mentoObj,
            mentoInfoText: ''
        ...
        return (
            ...
            { wholeFollow.map((friend, index) => (
                <div className = "friend-box">
                    ...
                    <div className = "friend-message-img" onClick = { () => this._Message_With(friend) }></div>
                </div>
            ))}
            </>
        )
    ...
    _renderMento = () => {
        ...
        const { follow } = this.state.userData;
        return(
            <>
                    ...
                    { mentoData.map((mento, index) => (
                        <>
                            <tr height="40px">
                                ...
                                <td>
                                    { !follow.includes(mento.mento) && mento.mento != this.state.username &&
                                        <button className="follow-button" onClick={() => this._addFriendFromMento(mento.mento)}>+ 팔로우</button>
                                    }
                                    { follow.includes(mento.mento) &&
                                        "팔로우 중"
                                    }
                                    { mento.mento == this.state.username &&
                                        "팔로워 받는 중"
                                    }
                                </td>
                            </tr>
    ...
    _addFriendFromMento = (newfriend) => {
        if (this.state.Login) {
            const { follow, name } = this.state.userData;
            const userObj = {
                ...this.state.userData,
                follow: [...follow, newfriend]
            }

            this.setState({ userData: userObj });
            window.sessionStorage.setItem("JITSIuser" + name, JSON.stringify(userObj));
        }
        else {
            this.pageFlip(4);
        }
        ...
        return (
            <div className="message-section">
                ...
                <div className = "send-message-section">
                    ...
                    <button className = "message-send-button" onClick={ () => this._sendMessage(messageObj, valueName, username, messageWith, this.state.messageText, divScroll)}>전송</button>
                </div>
            </div>
        )
    }
    ...
    _sendMessage(messageObj, _valueName, _sender, _receiver, _text, divScroll) {
        console.log("receiver : ", _receiver);
        if (this.state.Login) {
            if (this.state.messageWith) {
                let _messageObj = [
                    ...messageObj,
                ...
                const { follow } = this.state.userData;
                if (follow.includes(_receiver)){

                    let _receiverObj = JSON.parse(window.sessionStorage.getItem("JITSIuser" + _receiver));
                    ...
                    else if (!_receiverObj.follower.includes(_sender)) {
                        _receiverObj.follower = [
                            ..._receiverObj.follower,
                            _sender
                        ]
                    }
                    window.sessionStorage.setItem("JITSIuser" + _receiver, JSON.stringify(_receiverObj));
                }
            }
        }
        else {
